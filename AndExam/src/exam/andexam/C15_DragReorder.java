...
public class C15_DragReorder extends Activity {
	// 로그용 태그
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.c15_dragreorder);
		...
		for (int i = 0; i < arTerm.length; i++) {
			...
			LinearLayout item = (LinearLayout)Inflater.inflate(R.layout.c15_listitem, null);
			...
			item.setOnClickListener(new View.OnClickListener() {
				public void onClick(View v) {
					...
					Toast.makeText(C15_DragReorder.this, title.getText(), 0).show();
				}
			});
			//*/
	...
	void prepareDragReorder() {
		...
		mParamsBar.width = WindowManager.LayoutParams.FILL_PARENT;
...
class ReorderScrollView extends ScrollView {
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		Log.d(C15_DragReorder.TAG, "onInterceptTouchEvent return = " + result);
		return result; 
	}
};
